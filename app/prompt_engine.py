...
class PromptEngine:
    ...
    def _initialize_hardcoded_queries(self) -> Dict[str, str]:
        ...
        return {
            ...
            "list citizens": "SELECT citizen_id, name, age, gender, address FROM citizens ORDER BY name LIMIT 20",
    ...
    def process_query(self, natural_language_query: str) -> Dict[str, Any]:
        """Enhanced natural language query to SQL conversion"""
        try:
            # Clean the input
            ...
            # Try hardcoded queries first (fastest)
            sql_result = self._match_hardcoded_query(cleaned_query)
            if sql_result:
                return self._format_success_response(
                    sql_result, "hardcoded", natural_language_query, cleaned_query, entities
                )
            
            # Try intelligent pattern matching with templates
            sql_result = self._intelligent_pattern_match(cleaned_query, entities, operations)
            if sql_result:
                return self._format_success_response(
                    sql_result, "intelligent_pattern", natural_language_query, cleaned_query, entities
                )
            
            # Try basic pattern matching (fallback)
            sql_result = self._pattern_match_query(cleaned_query)
            if sql_result:
                return self._format_success_response(
                    sql_result, "basic_pattern", natural_language_query, cleaned_query, entities
                )
            
            # Future: GPT integration point
            if self.use_gpt and self.gpt_api_key:
                sql_result = self._gpt_process_query(cleaned_query, entities, operations)
                if sql_result:
                    return self._format_success_response(
                        sql_result, "gpt", natural_language_query, cleaned_query, entities
                    )
            
            # No match found - provide helpful suggestions
            return {
                "status": "error",
                "message": "Could not convert query to SQL. Please try rephrasing or use one of the suggested queries.",
                "original_query": natural_language_query,
                "detected_entities": entities,
                "detected_operations": operations,
                "suggestions": self._get_intelligent_suggestions(entities, operations),
                "sample_queries": self._get_contextual_samples(entities)
            }
    ...
    def _gpt_process_query(self, query: str, entities: List[str], operations: List[str]) -> Optional[str]:
        """Future GPT integration point for complex query processing"""
        # This is a placeholder for future GPT integration
        # When implemented, this will:
        # 1. Create a context-aware prompt with schema information
        # 2. Include examples of successful query conversions  
        # 3. Send to OpenAI GPT API for natural language to SQL conversion
        # 4. Validate and sanitize the generated SQL
        
        if not self.gpt_api_key:
            logger.info("GPT integration requested but no API key configured")
            return None
        ...
        logger.info("GPT integration is prepared but not yet implemented")
        return None
